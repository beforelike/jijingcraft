...
当前项目的优势是已有较完整的“规则决策 + Mineflayer 执行”骨架：`SurvivalController` 覆盖吃食物、采木、制作、采石、石器升级、狩猎、庇护所、夜间等待、撤离、反击、记忆、坑洞逃生和失败学习等动作，但运行测试说明这些动作必须继续加强结果验证和失败兜底。`decision.js` 是确定性策略入口，`memoryStore.js` 是本地生存记忆，`progress.js` 是阶段进度模型。
